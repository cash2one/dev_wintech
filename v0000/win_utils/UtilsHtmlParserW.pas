...
function ParserHtmlW(const Source: WideString): IHtmlElement; stdcall;
...
type
  TStringDynArray = array of WideString;
...
{$IFNDEF USE_GENERICS}
  ...
  public
    ...
    destructor Destroy; override;
    procedure Add(const Key: WideString; Value: WideString);
    procedure AddOrSetValue(const Key: WideString; Value: WideString);
    procedure Clear;
    procedure Remove(const Key: WideString);
    function Modify(const Key: WideString; Value: WideString): Boolean;
    function ValueOf(Key: WideString): WideString;
    function ContainsKey(const Key: WideString): Boolean;
    function GetKeys(): TStringDynArray;

    property Values[Key: WideString]: WideString read ValueOf; default;
  end;
{$ELSE}
...
{$IFEND}
  ...
  THtmlElement = class(TInterfacedObject, IHtmlElement, IHtmlElementObj)
    ...
    constructor Create(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsText(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsScript(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsStyle(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsComment(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsTag(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    constructor CreateAsDocType(AOwner: THtmlElement; AText: WideString;
      ALine, ACol: Integer);
    destructor Destroy; override;
...
procedure TStringDictionary.Add(const Key: WideString; Value: WideString);
var
  Hash: Integer;
  Bucket: PHashItem;
begin
...
procedure TStringDictionary.AddOrSetValue(const Key: WideString; Value: WideString);
begin
...
function TStringDictionary.ContainsKey(const Key: WideString): Boolean;
begin
  Result := Find(Key)^ <> nil;
end;
...
function TStringDictionary.Find(const Key: WideString): PPHashItem;
var
  Hash: Integer;
begin
  Hash := HashOf(Key) mod Cardinal(Length(Buckets));
...
function TStringDictionary.HashOf(const Key: WideString): Cardinal;
var
  I: Integer;
begin
  Result := 0;
...
function TStringDictionary.Modify(const Key: WideString; Value: WideString): Boolean;
var
  P: PHashItem;
begin
  P := Find(Key)^;
...
procedure TStringDictionary.Remove(const Key: WideString);
var
  P: PHashItem;
  Prev: PPHashItem;
begin
...
function TStringDictionary.ValueOf(Key: WideString): WideString;
var
  P: PHashItem;
begin
  P := Find(Key)^;
...
procedure DoError(const Msg: WideString);
begin
...
function TrimStr(ACharSet: TSysCharSet; const Str: WideString): WideString;
var
...
begin
  ...
  begin
    if not CharInSetW(Str[I], ACharSet) then
    begin
      if _begin < 0 then
        _begin := I;
      _End := I;
...
function SplitStr(ACharSet: TSysCharSet; AStr: WideString): TStringDynArray;
var
  L, I: Integer;
  S: WideString;
  StrChar: WideChar;
begin
  ...
  begin
    if CharInSetW(AStr[I], ['''', '"']) then
      if StrChar = #0 then
        StrChar := AStr[1]
      else if StrChar = AStr[1] then
        StrChar := #0;
    if StrChar = #0 then
      if CharInSetW(AStr[I], ACharSet) then
      begin
        if I > L then
        begin
          S := Copy(AStr, L, I - L);
...
procedure _SkipBlank(var P: PWideChar);
begin
  while CharInSetW(P^, WhiteSpace) do
    Inc(P);
end;

function _ReadStr(var P: PWideChar; UntilChars: TSysCharSet): WideString;
var
  oldP: PWideChar;
  stringChar: WideChar;
begin
  ...
  begin
    ...
    begin
      if CharInSetW(P^, UntilChars) then
        Break;
    end
...
procedure _ParserAttrs(P: PWideChar; var Attrs: TAttributeDynArray);
var
  Key, Value: WideString;
begin
  SetLength(Attrs, 0);
...
procedure _ParserNodeItem(S: WideString; var ATagName: WideString;
...
function ConvertEntities(S: WideString): WideString; forward;
function ConvertWhiteSpace(S: WideString): WideString; forward;
function GetTagProperty(const TagName: WideString): Word; forward;
function ParserCSSSelector(const Value: WideString): TCSSSelectorItemGroup; forward;

procedure _ParserHTML(const Source: WideString; AElementList: THtmlElementList);
var
  BeginLineNum, LineNum, BeginColNum, ColNum: Integer;

  procedure IncSrc(var P: PWideChar); overload;
  begin
    if P^ = #10 then
    begin
      Inc(LineNum);
  ...
  procedure IncSrc(var P: PWideChar; step: Integer); overload;
  var
    I: Integer;
  begin
  ...
  function PeekStr(const P: PWideChar): WideString;
  var
    Lp: PWideChar;
  begin
    Result := '';
    Lp := P;
    while not CharInSetW(Lp^, (WhiteSpace + ['/', '>'])) do
      Inc(Lp);
    SetString(Result, P, Lp - P);
  end;

  function IsEndOfTag(P: PWideChar; TagName: WideString): Boolean;
  begin
    Result := false;
    Inc(P);
    if P^ <> '/' then
  ...
  function PosCharInTag(var P: PWideChar; AChar: WideChar): Boolean;
  var
    StrChar: WideChar;
  begin
    Result := false;
  ...
  function ParserStyleData(var P: PWideChar): WideString;
  var
    oldP: PWideChar;
  begin
    oldP := P;
  ...
  function ParserScriptData(var P: PWideChar): WideString;
  var
    oldP: PWideChar;
    stringChar: WideChar;
    PreIsblique: Boolean;
...
constructor THtmlElement.Create(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
  inherited Create();
  FSourceLine := ALine;
...
constructor THtmlElement.CreateAsComment(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
...
constructor THtmlElement.CreateAsDocType(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
  Create(AOwner, AText, ALine, ACol);
  FContent := ConvertEntities(AText);
...
constructor THtmlElement.CreateAsScript(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
  Create(AOwner, AText, ALine, ACol);
  FContent := ConvertEntities(AText);
...
constructor THtmlElement.CreateAsStyle(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
  Create(AOwner, AText, ALine, ACol);
  FContent := ConvertEntities(AText);
...
constructor THtmlElement.CreateAsTag(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
var
...
constructor THtmlElement.CreateAsText(AOwner: THtmlElement; AText: WideString;
  ALine, ACol: Integer);
begin
  Create(AOwner, AText, ALine, ACol);
  FContent := ConvertEntities(AText);
...
//
function ParserCSSSelector(const Value: WideString): TCSSSelectorItemGroup;
var
  LineNum, ColNum: Integer;
  procedure IncSrc(var P: PWideChar);
  begin
    if P^ = #10 then
    begin
      Inc(LineNum);
  ...
  function ReadStr(var P: PWideChar; Chars: TSysCharSet): WideString;
  var
    oldP: PWideChar;
  begin
    oldP := P;
    while not(CharInSetW(P^, Chars)) do
      IncSrc(P);
    SetString(Result, oldP, P - oldP);
  end;
  ...
  function ParserAttr(var P: PWideChar): TAttrSelectorItem;
  var
    oldP: PWideChar;
    Tmp: WideString;
    stringChar: WideChar;
  begin
    ...
    SkipBlank(P);
    oldP := P;
    while not CharInSetW(P^, (WhiteSpace + OperatorChar + [']', #0])) do
      IncSrc(P);
    ...
    begin
      if stringChar = #0 then
      begin
        if CharInSetW(P^, (WhiteSpace + [#0, ']'])) then
          Break;
      end
      else if (P^ = stringChar) then
      begin
  ...
  procedure ParserItem(var P: PWideChar; var Item: TCSSSelectorItem);
  var
//    Tmp: WideString;
...
function ConvertEntities(S: WideString): WideString;
var
  I: Integer;
//  J: Integer;
  T: WideString;
...
function ConvertWhiteSpace(S: WideString): WideString;
var
  Sb: TStringBuilder;
  I: Integer;
...
begin
  ...
  begin
    ThisIsWhite := CharInSetW(S[I], WhiteSpace);
    if ThisIsWhite then
    begin
...
const
  BlockTags: array [0 .. 59 - 1] of WideString = ('HTML', 'HEAD', 'BODY',
    ...
    'MENU', 'PLAINTEXT');
  InlineTags: array [0 .. 56 - 1] of WideString = ('OBJECT', 'BASE', 'FONT', 'TT',
  ...
  EmptyTags: array [0 .. 14 - 1] of WideString = ('META', 'LINK', 'BASE', 'FRAME',
    'IMG', 'BR', 'WBR', 'EMBED', 'HR', 'INPUT', 'KEYGEN', 'COL', 'COMMAND',
    'DEVICE');
  FormatAsInlineTags: array [0 .. 19 - 1] of WideString = ('TITLE', 'A', 'P', 'H1',
    'H2', 'H3', 'H4', 'H5', 'H6', 'PRE', 'ADDRESS', 'LI', 'TH', 'TD', 'SCRIPT',
    'STYLE', 'INS', 'DEL', 'S');
  PreserveWhitespaceTags: array [0 .. 4 - 1] of WideString = ('PRE', 'PLAINTEXT',
    'TITLE', 'TEXTAREA');

var
  GTagProperty: TStringDictionary;

function GetTagProperty(const TagName: WideString): Word;
var
  Key, S: WideString;
begin
  Result := 0;
...
begin
  ...
  begin
    ...
    if GTagProperty.ContainsKey(Key) then
      ...
      S := #0;
    S[1] := WideChar(Ord(S[1]) or TpEmpty);
  ...
  begin
    ...
    if GTagProperty.ContainsKey(Key) then
      ...
      S := #0;
    S[1] := WideChar(Ord(S[1]) or TpFormatAsInline);
  ...
  begin
    ...
    if GTagProperty.ContainsKey(Key) then
      ...
      S := #0;
    S[1] := WideChar(Ord(S[1]) or TpPreserveWhitespace);
