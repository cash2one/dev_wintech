...
var
  SaveSeparator: Char;
begin
  SaveSeparator := {$IF CompilerVersion >= 21.0}FormatSettings.{$IFEND}DecimalSeparator;
  {$IF CompilerVersion >= 21.0}FormatSettings.{$IFEND}DecimalSeparator := '.';
  try
    Result := FloatToStr(Value);
  finally
    {$IF CompilerVersion >= 21.0}FormatSettings.{$IFEND}DecimalSeparator := SaveSeparator;
  end;
end;
{$ENDIF}
