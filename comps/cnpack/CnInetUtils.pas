...
* 单元标识：$Id: CnInetUtils.pas 1146 2012-10-24 06:25:41Z liuxiaoshanzhashu@gmail.com $
...
function CnInet_GetStreamFromHandle(AInet: TCnInet; Handle: HINTERNET; TotalSize: Integer;
  Stream: TStream): Boolean;
function GetHTTPStream(AInet: TCnInet; Info: TCnURLInfo; Stream: TStream; APost: TStrings): Boolean;
function GetFTPStream(AInet: TCnInet; Info: TCnURLInfo; Stream: TStream): Boolean;  
...
const
  ...
  begin
    ...
    repeat
      SetLength(s, cbSize);
      hRet := ObtainUserAgentString(0, PAnsiChar(s), cbSize);
      case hRet of
        E_OUTOFMEMORY:
          cbSize := cbSize * 2;
        NOERROR:
...
begin
  ...
  if SameText(Info.Protocol, 'http') or SameText(Info.Protocol, 'https') then
    Result := GetHTTPStream(Self, Info, Stream, APost)
  else if SameText(Info.Protocol, 'ftp') then
    Result := GetFTPStream(Self, Info, Stream);
...
begin
  ...
  repeat
    ...
    begin
      ...
      if AInet <> nil then
      begin
        AInet.DoProgress(TotalSize, CurrSize);
        if AInet.Aborted then Exit;
      end;
    end;
  until Readed = 0;
  Result := True;
...
begin
  ...
  hFtp := nil;
  try
    hConnect := InternetConnect(AInet.hSession, PChar(Info.Host),
      StrToIntDef(Info.Port, INTERNET_DEFAULT_FTP_PORT),
      PChar(Info.Username), PChar(Info.Password),
      INTERNET_SERVICE_FTP, 0, 0);
    if (hConnect = nil) or AInet.FAborted then
      Exit;
    ...
    if (hFtp = nil) or AInet.FAborted then
      Exit;
      
    Result := CnInet_GetStreamFromHandle(AInet, hFtp, TotalSize, Stream);
  finally
...
begin
  ...
  try
    ...
    if AInet.FNoCookie then
      Flag := Flag + INTERNET_FLAG_NO_COOKIES;

    hConnect := InternetConnect(AInet.hSession, PChar(Info.Host), Port, nil, nil,
    ...
    if AInet.EncodeUrlPath then
      PathName := EncodeURL(PathName);
    hRequest := HttpOpenRequest(hConnect, PChar(Verb), PChar(PathName),
      HTTP_VERSION, nil, nil, Flag, 0);
    if (hRequest = nil) or AInet.FAborted then
      Exit;

    if AInet.FDecoding and AInet.FDecodingValid then
    ...
    for i := 0 to AInet.FHttpRequestHeaders.Count - 1 do
      HttpAddRequestHeaders(hRequest, PChar(AInet.FHttpRequestHeaders[i]),
        Length(AInet.FHttpRequestHeaders[i]), HTTP_ADDREQ_FLAG_REPLACE or HTTP_ADDREQ_FLAG_ADD);

    if HttpSendRequest(hRequest, nil, 0, POpt, OptLen) then
    begin
      if AInet.FAborted then Exit;
      ...
      Result := CnInet_GetStreamFromHandle(AInet, hRequest, StrToIntDef(SizeStr, -1), Stream);
    end;
  finally
