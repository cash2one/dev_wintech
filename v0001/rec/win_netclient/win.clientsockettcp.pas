...
type
  PWinNetTcpClient  = ^TWinNetTcpClient;
  TWinNetTcpClient  = record
    Base            : TWinNetClient;
  end;

  function CheckOutTcpClient: PWinNetTcpClient;
  procedure CheckInTcpClient(var AClient: PWinNetTcpClient);
  
  function TcpClientConnect(ATcpClient: PWinNetTcpClient; AConnectAddress: PNetServerAddress): Boolean;
  procedure TcpClientDisconnect(ATcpClient: PWinNetTcpClient);
  procedure TcpClientSetReadTimeOut(ANetClient: PWinNetTcpClient; const Value: Integer);

implementation

uses
  Windows, WinSock2, win.net;

function CheckOutTcpClient: PWinNetTcpClient;
begin
  Result := System.New(PWinNetTcpClient);
  FillChar(Result^, SizeOf(TWinNetTcpClient), 0);  
